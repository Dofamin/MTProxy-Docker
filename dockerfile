...
# CMD
CMD ["-c", "/MTProxy/objs/bin/mtproto-proxy", "-u", "nobody", "-p", "8888", "-H", "443", "-S", $Secret, "--aes-pwd", "proxy-secret", "proxy-multi.conf", "-M", $Workers, "--nat-info",  "$(getent hosts $(cat /etc/hostname) | awk '{print $1; exit}'):$(curl ifconfig.co)", "--http-stats"]
