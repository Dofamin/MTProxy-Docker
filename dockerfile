...
# CMD
CMD ["bash" "/MTProxy/objs/bin/mtproto-proxy -u nobody -p 8888 -H 443 -S $Secret --aes-pwd /MTProxy/objs/bin/ proxy-secret /MTProxy/objs/bin/proxy-multi.conf -M $Workers --nat-info $(getent hosts $(cat /etc/hostname) | awk '{print $1; exit}'):$(curl ifconfig.co) --http-stats"]
